classdef Software_SC_Executive < handle
    ...
    methods
        function obj = Software_SC_Executive(True_SC_body,True_time,software_SC_communication)
            ...
            obj.wait_time_comm_interSC = 3*60*60;
            obj.wait_time_comm_dte = 24*60*60;
        ...
        function obj = func_schedule_main_tasks(obj,mission_true_SC)
            ...
            % Get Camera Images
            if (obj.time - mission_true_SC.true_SC_camera_sensor.time) >= obj.wait_time_get_camera_image
                obj.get_camera_image = obj.requested_get_camera_image; % Get the value from the MEXEC interface
            else
                obj.get_camera_image = 0;
            end
            
            % Check if SB visible on any of the cameras
            ...
            if flag_SB_visible_on_camera == 0
                obj.time_SB_visible = inf;
            else
                if obj.time_SB_visible == inf
                    obj.time_SB_visible = obj.time;
                end
            end
            
            % Control orbit
            if ((obj.time - mission_true_SC.software_SC_control_orbit.time) >= obj.wait_time_control_SC_orbit) && (obj.time - obj.time_SB_visible > 400) % (norm(SC_estimate_orbits.SB_position_uncertainty) <= 1) && (norm(SC_estimate_attitude.angular_velocity - SC_control_attitude.desired_angular_velocity) < 1e-3)
            ...
            if ( ( (obj.control_SC_orbit == 0) && ((obj.time - mission_true_SC.software_SC_control_orbit.time) < obj.wait_time_control_SC_orbit) ) && ((obj.time - mission_true_SC.software_SC_communication.time_interSC_comm) >= obj.wait_time_comm_interSC) ) && (mission_true_SC.true_SC_body.flag_hardware_exists.intersat_communication == 1)
                ...
                obj.send_data_comm = ones(1,mission_true_SC.software_SC_communication.number_communication); % trigger all intersat comm, they will be organized in communication class
            elseif (mission_true_SC.true_SC_body.flag_hardware_exists.dte_communication == 1) && (sum(mission_true_SC.software_SC_communication.data_needs_to_be_exchanged) == 0) && (obj.control_SC_orbit == 0) && ((obj.time - mission_true_SC.software_SC_control_orbit.time) < obj.wait_time_control_SC_orbit) && ((obj.time - mission_true_SC.software_SC_communication.time_dte_comm) >= obj.wait_time_comm_dte || ((mission_true_SC.software_SC_communication.data_storage/mission_true_SC.software_SC_communication.maximum_data_storage) > obj.Data_max_threshold))
                % DTE communication
                % [ Condition of activation ] :
                %      * no orbit control running
            ...
            for i=1:1:mission_true_SC.true_SC_battery.num_battery
                mean_battery_state_of_charge = mean_battery_state_of_charge + ((1/mission_true_SC.true_SC_battery.num_battery) * mission_true_SC.true_SC_battery.battery_data(i).state_of_charge);
            end
            ...
            elseif (mission_true_SC.software_SC_control_orbit.desired_DeltaV_needs_to_be_executed == 1) && (obj.time >= mission_true_SC.software_SC_control_orbit.time_data_cutoff)
                obj.desired_SC_attitude_mode = 3;
            elseif (obj.send_data_to_Earth == 1)
            ...
            elseif (obj.send_data_to_Earth == 0) && (sum(obj.send_data_comm) == 1)
                ...
                index_comm = find(obj.send_data_comm == 1);
                if mission_true_SC.software_SC_communication.comm_needs_attitude_pointing(index_comm) == 1
                    % if attitude pointing needed for this comm
            ...
            if mission_true_SC.true_SC_body.flag_hardware_exists.adc_reaction_wheel_assembly == 1
                for i=1:mission_true_SC.true_SC_rwa_actuator.num_reaction_wheel
                    if abs(mission_true_SC.true_SC_rwa_actuator.RW_data(i).true_angular_velocity) >= obj.RW_desat_threshold*mission_true_SC.true_SC_rwa_actuator.RW_data(i).maximum_angular_velocity
