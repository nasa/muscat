classdef Software_SC_Estimate_Attitude_MEKF < handle
    ...
    methods
        ...
        function obj = Software_SC_Estimate_Attitude_MEKF(this_true_SC,mission_true_time)
            % MULTIPLICATIVE EXTENDED KALMAN FILTER
            % The MEKF utilizes a quaternion to estimate the absolute spacecraft attitude and a set of Euler angles to
            % represent the attitude error computed in the measurement update. This architecture avoids violating the
            % quaternion unit norm constraint.
            %
            % Reference: Beierle et al., Two-stage attitude control for direct imaging of exoplanets with a CubeSat telescope
            ...
            obj.moment_of_inertia = this_true_SC.true_SC_adc.moment_of_intertia.total_MI;
            
            % Initialize KF
            % The state vector, x, is composed of small angles, α, which have a zero mean expectation.
            % x = [alpha_x, alpha_y, alpha_z]
            obj.x = zeros(3,1);
            obj.P = zeros(7,7);
            
            if this_true_SC.true_SC_body.flag_hardware_exists.adc_imu==1
                obj.P_km1_km1(1:3,1:3) = this_true_SC.true_SC_imu_sensor.imu_sensor_data(1).measurement_noise*eye(3,3);
            end
            if this_true_SC.true_SC_body.flag_hardware_exists.adc_sun_sensor==1
                obj.P_km1_km1(4:7,4:7) = this_true_SC.true_SC_sun_sensor.sun_sensor_data(1).measurement_noise*eye(4,4);
            end
            ...
            obj = func_compute_rotation_matrix(obj, this_true_SC.true_SC_adc);
        end
        ...
        function obj = func_KF_update_step(obj,True_SC_IMU_Sensor,True_SC_Sun_Sensor,True_SC_Star_Tracker_Sensor)
            ...
            if obj.measurement_available_imu || obj.measurement_available_star_tracker || obj.measurement_available_sun_sensor
        ...
        function obj = func_compute_rotation_matrix(obj,SC_control_attitude)
            ...
            % Compute sc_omega_dot
            new_X_Quaternion_Omega_current = [obj.attitude; obj.angular_velocity];
            X_dot = func_attitude_Quaternion_spacecraft_v2(obj.time,new_X_Quaternion_Omega_current,obj.moment_of_inertia,SC_control_attitude.control_torque,[0 0 0]');
