%% Class: True_SC_EP__Thruster
% Individual SC's electric propulsion thruster

classdef True_SC_EP_Thruster < handle

    % True_SC_EP_Thruster class represents a single electric propulsion thruster
    ...
    properties
        ...
        gimbal_noise                     %[deg] : Added to every dimension of commanded_gimbal
        ...
        orientation_polar                %[deg] : Azimuth and Elevation of default orientation measured based on body frame B coordinates
        location                         %[m] : Location of actuator, in body frame B

        maximum_thrust                   %[N]
        minimum_thrust                   %[N] : This need not be 0

        maximum_gimbal                   %[deg] : Maximum gimbal angle (in each direction) of the thruster
        ...
        mode_true_EP_thruster_selector % Mode (Truth/Simple)
        ...
        pending_gimbal                   %[deg] : Remembered gimbal angles for a pending fire command
        ...
        commanded_gimbal                 %[deg] : Gimbal angle commanded by the SC's orbit control software

        true_commanded_gimbal            %[deg] : Actual gimbal angle with noise applied, unknown to SC software
    ...
    methods
        ...
        function obj = True_SC_EP_Thruster(init_data, mission, i_SC, i_MT)
            if isfield(init_data, 'name')
                obj.name = init_data.name;
            else
                obj.name = ['EP Thruster ', num2str(i_MT)];
            end
            obj.health = true;
            obj.temperature = 20; 
            ...
            obj.thruster_noise = init_data.thruster_noise; % Noise level
            obj.gimbal_noise = init_data.gimbal_noise;
            ...
            obj.orientation_polar = [0 0];
            [obj.orientation_polar(1), obj.orientation_polar(2), ~] = cart2sph(obj.orientation(1), obj.orientation(2), obj.orientation(3));
            obj.orientation_polar = rad2deg(obj.orientation_polar);
            ...
            obj.maximum_gimbal = init_data.maximum_gimbal; % degrees
            ...
            obj.commanded_gimbal = zeros(1,2); % Initialize as 1x2 array
            obj.true_commanded_gimbal = zeros(1,2); % Initialize as 1x2 array
            ...
            obj.store.commanded_gimbal = zeros(mission.storage.num_storage_steps,2);
            ...
            % Update storage`
            func_update_true_EP_thruster_store(obj, mission);
        ...
        function obj = func_update_true_EP_thruster_store(obj, mission)
            ...
            obj.store.commanded_gimbal(mission.storage.k_storage,:) = obj.commanded_gimbal;
            obj.store.true_commanded_gimbal(mission.storage.k_storage,:) = obj.true_commanded_gimbal;
        ...
        function obj = func_validate_thruster_properties(obj)
            ...
            if isempty(obj.commanded_gimbal)
                obj.commanded_gimbal = zeros(1,2);
            end

            if isempty(obj.true_commanded_gimbal)
                obj.true_commanded_gimbal = 0;
            end
        ...
        function func_main_true_ep_thruster(obj, mission, i_SC, ~)
            ...
            switch obj.thruster_state
                ...
                case 'ready'
                    ...
                    if obj.flag_executive && obj.commanded_thrust > 0
                        ...
                        obj.pending_gimbal = obj.commanded_gimbal;

                        disp(['Thruster transitioning from ready to firing at time ', num2str(mission.true_time.time), ' sec']);
                        disp(['  Commanded thrust: ', num2str(obj.commanded_thrust), ' N with gimbal angles of ', num2str(obj.commanded_gimbal), 'degrees']);
                    ...
                    if (obj.flag_executive && obj.commanded_thrust > 0) || obj.pending_fire
                        % try
                            ...
                            if obj.flag_executive && obj.commanded_thrust > 0
                                ...
                                gimbal_to_use = obj.commanded_gimbal;
                            else
                                thrust_to_use = obj.pending_thrust;
                                gimbal_to_use = obj.pending_gimbal;
                                disp(['Using pending thrust from previous cycle: ', num2str(thrust_to_use), ' N with gimbal angles of ', num2str(gimbal_to_use), ' degrees']);
                            end
                            ...
                            % Update gimbal with noise
                            obj.true_commanded_gimbal = gimbal_to_use + obj.gimbal_noise * (2*rand(1,2) - 1);

                            true_gimbal = deg2rad(obj.true_commanded_gimbal + obj.orientation_polar);
                            ...
                            % Get the transformation matrix with gimbal angles
                            % Rgim = GimballedToBodyRotationMatrix(obj.true_commanded_gimbal);
        
                            % Calculate force vector in body frame first
                            % force_body = (Rgim * (obj.true_commanded_thrust * obj.orientation)')';
                            force_body = [0 0 0];
                            [force_body(1), force_body(2), force_body(3)] = sph2cart(true_gimbal(1), true_gimbal(2), obj.true_commanded_thrust);

                            % Transform to inertial frame
                            obj.force_inertial = R * force_body'; % Netwons
                            
                            % Log the thruster firing 
                            %disp(['THRUSTER FIRING with thrust of ', num2str(obj.true_commanded_thrust), ' N with gimbal angles of ', num2str(gimbal_to_use), ' degrees at time ', num2str(mission.true_time.time), ' sec']);
                            ...
                            % Update power consumption (actuation)
            ...
            % Update storage
            obj.func_update_true_EP_thruster_store(mission);
            ...
            obj.true_commanded_thrust = 0;
            obj.true_commanded_gimbal = zeros(1,2);
        end
    end
end
