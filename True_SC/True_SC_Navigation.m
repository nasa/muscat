...
classdef True_SC_Navigation < handle
    ...
    properties
        ...
        position % [km] : Current position of SC wrt Solar System Barycenter centered J2000
        velocity % [km/sec] : Current velocity of SC wrt Solar System Barycenter centered J2000
        position_relative_target % [km] : Current position of SC relative to Target-center J2000 inertial frame
        velocity_relative_target % [km/sec] : Current velocity of SC relative to Target-center J2000 inertial frame
    ...
    methods
        ...
        function obj = True_SC_Navigation(init_data, mission)
            ...
            elseif strcmp(mission.frame,'Relative') || isfield(init_data, 'position_relative_target')
                obj.position_relative_target = init_data.position_relative_target; % [km]
                obj.velocity_relative_target = init_data.velocity_relative_target; % [km/sec]
                obj.name_relative_target = init_data.name_relative_target; % [string]

                for i_target = 1:1:mission.num_target
                    if strcmp(mission.true_target{i_target}.name, obj.name_relative_target)
                        obj.index_relative_target = i_target;
                        position_target = mission.true_target{obj.index_relative_target}.position; % [km]
                        velocity_target = mission.true_target{obj.index_relative_target}.velocity; % [km/sec]
                    end
                end
        ...
        function obj = func_main_true_SC_navigation(obj, mission, i_SC)
            ...
            switch obj.mode_true_SC_navigation_dynamics_selector
                ...
                case 'Relative Dynamics'
                    % Use Relative Dynamics
                    obj = func_update_SC_navigation_position_velocity_Relative_Dynamics(obj, mission, i_SC);
        ...
        function obj = func_update_SC_navigation_position_velocity_SPICE(obj, mission)
            ...
            SC_pos_vel = cspice_spkezr(obj.spice_name,mission.true_time.date,'J2000','NONE','SOLAR SYSTEM BARYCENTER');
            ...
            SC_pos_vel_array = (cspice_spkezr(obj.spice_name, mission.true_time.prev_date + mission.true_time.time_position_array' ,'J2000','NONE','SOLAR SYSTEM BARYCENTER'))';
        ...
        function obj = func_update_SC_navigation_position_velocity_Absolute_Dynamics(obj, mission, i_SC)
            ...
            [T,X]=ode15s(@(t,X) func_ode_orbit_inertial_absolute_dynamics(t, X, mission, i_SC), this_time_array, SC_pos_vel_current, obj.ode_options);
        ...
        %% [ ] Methods: Update Position Velocity Relative Dynamics
        % Update position and velocity using Relative dynamics

        function obj = func_update_SC_navigation_position_velocity_Relative_Dynamics(obj, mission, i_SC)

            SC_pos_vel_current_realtive = [obj.position_relative_target'; obj.velocity_relative_target']; % [km, km/sec] in (6,1) format
            [T,X]=ode15s(@(t,X) func_orbit_inertial_static_relative_Target(t,X, mission, i_SC, obj.index_relative_target), mission.true_time.time_position_array, SC_pos_vel_current_realtive, obj.ode_options);

            new_SC_pos_vel_current_relative = X(end,:);

            obj.position_relative_target = new_SC_pos_vel_current_relative(1:3); % [km]
            obj.velocity_relative_target = new_SC_pos_vel_current_relative(4:6); % [km/sec]
            ...
            if length(mission.true_time.time_position_array) == 2
                obj.position_array(1,1:3) = mission.true_target{obj.index_relative_target}.position_array(1,1:3) + X(1,1:3); % [km]
                obj.position_array(2,1:3) = mission.true_target{obj.index_relative_target}.position_array(2,1:3) + X(end,1:3); % [km]
            else
                obj.position_array = mission.true_target{obj.index_relative_target}.position_array + X(:,1:3); % [km]
            end
        ...
        function obj = func_update_visible_Sun_Earth(obj, mission)
            ...
            for i_vis = 1:1:2
                ...
                elseif (i_vis == 2) && (mission.true_solar_system.index_Earth ~= 0)
                    % Earth is a SS Body
                    x1 = mission.true_solar_system.SS_body{mission.true_solar_system.index_Earth}.position; % [km]

                elseif (i_vis == 2) && (mission.true_solar_system.index_Earth == 0) && strcmp(mission.true_target{obj.index_relative_target}.name, 'Earth')
                    % Earth is the Target Body
                    x1 = mission.true_target{obj.index_relative_target}.position; % [km]
                ...
                for i_target = 1:1:mission.num_target
                    ...
                    if (i_vis == 2) && (mission.true_solar_system.index_Earth == 0) && strcmp(mission.true_target{i_target}.name, 'Earth')
                        % Skip this!
