...
classdef True_SC_SRP < handle
    ...
    methods
        ...
        function obj = True_SC_SRP(init_data, mission, i_SC)

            % Initialize SRP enable flag
            obj.enable_SRP = init_data.enable_SRP;

            % Initialize disturbance vectors
            obj.disturbance_torque_SRP = zeros(1,3);
            obj.disturbance_force_SRP = zeros(1,3);
            ...
            obj.func_main_true_SC_SRP(mission, i_SC);

            % Initialize storage
            obj.store.disturbance_torque_SRP = zeros(mission.storage.num_storage_steps, length(obj.disturbance_torque_SRP));
            obj.store.disturbance_force_SRP = zeros(mission.storage.num_storage_steps, length(obj.disturbance_force_SRP));
        ...
        %% [ ] Methods: Main
        % Update SRP Disturbance Force and Torque

        function obj = func_main_true_SC_SRP(obj, mission, i_SC)

            % Reset disturbance terms
            obj.disturbance_torque_SRP = [0 0 0];
            obj.disturbance_force_SRP = [0 0 0];
            ...
            if obj.enable_SRP == 1
                ...
                % Update disturbance Force and Torque
                mission.true_SC{i_SC}.true_SC_navigation.disturbance_force = mission.true_SC{i_SC}.true_SC_navigation.disturbance_force + obj.disturbance_force_SRP;
