function func_plot_individual_power_consumption(mission, i_SC)
...
%% Individual Power Consumption Visualization - Main Figure
% Create figure with specific tag for callback functionality
plot_handle = figure('Tag', 'PowerConsumptionFigure', 'Name', ['SC ', num2str(i_SC), ' Individual Hardware Power Consumption']);
clf
...
% Check if we have power data available
if ~isfield(mission.true_SC{i_SC}, 'true_SC_power') || ~isfield(mission.true_SC{i_SC}.true_SC_power.store, 'list_HW_energy_consumed')
    % No power data available - just show an empty plot with a message
    text(0.5, 0.5, 'No power consumption data available', 'HorizontalAlignment', 'center');
    axis off;
    return;
end

% Get the list of hardware and corresponding power consumption
hw_list = mission.true_SC{i_SC}.true_SC_power.store.list_HW_energy_consumed;
num_hw = length(hw_list);

if num_hw == 0
    % No hardware in the list - just show an empty plot with a message
    text(0.5, 0.5, 'No hardware components with power consumption data', 'HorizontalAlignment', 'center');
    axis off;
    return;
end

% Get hardware power consumption data
hw_power_data = mission.true_SC{i_SC}.true_SC_power.store.array_HW_energy_consumed(1:kd, :);

% Sort hardware items by total energy consumption (for better organization)
total_hw_consumption = sum(hw_power_data, 1);
[sorted_consumption, sort_idx] = sort(total_hw_consumption, 'descend');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % Create a 2x3 layout with subplots % %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:num_hw
    ...
    % Safe color indexing
    color_index = mod(i-1, size(color_array, 1)) + 1;
    
    % Use scatter for better performance with large datasets
    line_handles(i) = scatter(downsampled_time, hw_power_data(plot_indices, hw_idx), 5, ...
    ...
    if length(plot_indices) > 1
        line(downsampled_time, hw_power_data(plot_indices, hw_idx), ...
...
xlabel('Time [sec]');
ylabel('Energy [Whr]');
title('All Hardware Power Consumption', 'FontSize', mission.storage.plot_parameters.title_font_size, ...
...
selected_handle = plot(time_data, zeros(length(time_data), 1), '-', 'LineWidth', 3, 'Color', [0, 0.5, 0, 1]);
grid on;
xlabel('Time [sec]');
ylabel('Energy [Whr]');
...
top_consumption = total_hw_consumption(top_hw_idx);
total_consumption = sum(total_hw_consumption);
other_consumption = total_consumption - sum(top_consumption);
...
if num_hw > num_pie_items
    ...
    pie_data = [top_consumption, other_consumption];
    pie_labels = [hw_list(top_hw_idx), {'Other'}];
else
    pie_data = top_consumption;
    pie_labels = hw_list(top_hw_idx);
end

% Calculate percentages for labeling
percentages = 100 * pie_data / total_consumption;
...
title('Total Energy by Component', 'FontSize', mission.storage.plot_parameters.standard_font_size, ...
    'FontName', mission.storage.plot_parameters.standard_font_type);

% Add overall title for the figure
annotation('textbox', [0.25, 0.96, 0.5, 0.03], ... 
    'String', ['SC ', num2str(i_SC), ' Individual Hardware Power Consumption'], ...
    'EdgeColor', 'none', ...
    'HorizontalAlignment', 'center', ...
    'FontSize', mission.storage.plot_parameters.title_font_size, ...
    'FontName', mission.storage.plot_parameters.standard_font_type);

% Store data for the listbox
setappdata(plot_handle, 'hw_list', hw_list);
setappdata(plot_handle, 'hw_data', hw_power_data);
...
if mission.storage.plot_parameters.flag_save_plots == 1
    ...
    output_file = [mission.storage.output_folder, mission.name, '_SC', num2str(i_SC), '_Individual_Power_Consumption.png'];
    try
...
function selectHardware(src, ~, fig_handle)
    ...
    % Update the plot - keep right plot linear 
    set(selected_plot, 'XData', time_data, 'YData', hw_data(:, hw_idx), 'Color', color);
    set(top_right_plot, 'YScale', 'linear'); % Keep it linear
...
function resetAllHighlights(~, ~, fig_handle)
    ...
    axes(top_right_plot);
    % Reset the plot data
    set(selected_plot, 'XData', time_data, 'YData', zeros(length(time_data), 1));
    ...
    title('Selected Hardware', 'FontSize', std_font_size, ...
        'FontName', std_font_type);
