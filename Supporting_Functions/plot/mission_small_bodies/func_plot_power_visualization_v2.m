function func_plot_power_visualization_v2(mission, i_SC)
    ...
    if isfield(mission.true_SC{i_SC}.true_SC_solar_panel{1}.store, 'instantaneous_power_generated') && ...
        ...
        for i_HW = 1:num_panels
            try
                ...
                warning('Solar panel %d data incomplete. Skipping 1.', i_HW);
            end
        end
        ...
        for i_HW = 1:num_panels
            try
                ...
                % Safe color indexing
                color_index = mod(i_HW, length(color_array)) + 1;
                ...
                warning('Solar panel %d data incomplete. Skipping 2.', i_HW);
            end
        end
...
if isfield(mission.true_SC{i_SC}, 'true_SC_battery') && ...
    ...
    for i_batt = 1:num_batteries
        try
            ...
            warning('Battery %d data incomplete. Skipping 1.', i_batt);
        end
    end
    ...
    for i_batt = 1:num_batteries
        try
            ...
            warning('Battery %d data incomplete. Skipping 2.', i_batt);
        end
    end
