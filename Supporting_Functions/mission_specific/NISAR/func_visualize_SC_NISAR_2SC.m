...
% Visualize SC Configuration for GoldenDome

function obj = func_visualize_SC_NISAR_2SC(obj, mission, is_realtime_update)
...
subplot_style = 1;
% 1 = Show individual SC attitude
% 2 = Show only the Earth from different orientations
...
if subplot_style == 1
    ...
    if mission.num_SC == 2
        ...
        subplot_factor = ceil(sqrt(mission.num_SC));

        for i_SC = 1:1:mission.num_SC

            %     subplot(plot_factor,2*plot_factor,mod(i_SC,plot_factor) - 1 + ceil(i_SC/plot_factor) )

            rem = mod(i_SC,subplot_factor);
            if rem == 0
                rem = subplot_factor;
            end

            quo = ceil(i_SC/subplot_factor) - 1;

            subplot(subplot_factor, 2*subplot_factor, (quo*2*subplot_factor) + rem)
...
for i_subplot_style = 1:1:subplot_style

    if i_subplot_style == 1
        subplot(1,2,2)
    else
        subplot(1,2,1)
    end
    ...
    if flag_show_radar > 0
        ...
        all_monostatic_observed_point = mission.true_SC{1}.true_SC_remote_sensing{flag_show_radar}.monostatic_observed_point;
        for i_SC = 2:1:mission.num_SC
            all_monostatic_observed_point = all_monostatic_observed_point + mission.true_SC{i_SC}.true_SC_remote_sensing{flag_show_radar}.monostatic_observed_point;
        end

        % Define a colormap
        cmap = jet(max(all_monostatic_observed_point)+1); % Use a colormap with max(obj.monostatic_observed_point)+1 colors
        % cmap(1,:) = [1 1 1]; % Set 1 to white
        cmap(1,:) = NaN*[1 1 1]; % Set 1 to transparent

        % Map the values to colors
        colors = cmap(1+all_monostatic_observed_point', :);
        ...
        if i_subplot_style == 1
            ...
            colorbar;
            caxis([0 max(all_monostatic_observed_point)+1]);
        end
    ...
    title(['All ',num2str(mission.num_SC),' SC in ', mission.true_target{i_target}.name,'-centered Frame'],'FontSize',mission.storage.plot_parameters.standard_font_size)
...
if (obj.plot_parameters.flag_save_video == 1) && (mission.flag_stop_sim == 0) && is_realtime_update
    ...
    % % targetSize = [1508, 3824];  % [height, width]
    % % targetSize = [3006, 6016];  % [height, width]
    % % targetSize = [3226, 6016];  % [height, width]
